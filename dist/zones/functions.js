//  Dependancies
import { listTimeZones } from 'timezone-support'; //  Utility Functions

import { getFormattedOffset, isUnique, replaceAllCharacters, roundDownToNearestFive, setTimeToTimeZone } from '../utilities/functions';

const getButtonValues = (year, month, day, hour, minute, region, zone, identifier) => {
  ...
  const formattedToTimeZone = setTimeToTimeZone(time, 'HH:mm a', formatToTimeZoneObject);
...
const getZonesObject = (button = {}) => {
  ...
  const zones = listTimeZones().filter(region => {
    let invalid = false;
    ...
    */
    ...
    const deprecated = ['asia/qostanay', // unknown
    ...
    const buttonValues = getButtonValues(year, month, day, hour, roundedMinute, region, zone, identifier);
    /*
...
const getRegions = zones => {
  const regions = zones.map(zone => zone.region).filter(isUnique).sort();
  return regions;
};

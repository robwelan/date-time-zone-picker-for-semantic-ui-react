...
import { Grid, Ref } from 'semantic-ui-react'; //  Components
...
import SelectedZone from './selected-zone'; //  Functions

import { getRegions, getZonesObject } from './functions'; //  Styles
...
class Zones extends React.Component {
  ...
  onChangeFilter(e) {
    ...
    if (input !== '') {
      filtered = work.filter(item => item.button.search.toLowerCase().includes(input.toLowerCase()));
    }
  ...
  showRegion(region) {
    return () => {
      ...
      const filtered = work.filter(item => item.region.toLowerCase() === region.toLowerCase());
      this.setState(prevState => {
        const newState = { ...prevState,
          selection: { ...prevState.selection,
            filter: ''
  ...
  render() {
    ...
    const {
      ...
      visible: {
        ...
        zone: visibleZone,
        control: {
          allLocations: visibleAllLocationsControl
        }
      },
      zones: {
        filtered: zonesMain,
    ...
    return React.createElement(React.Fragment, null, allowZone && showZone && React.createElement(Ref, {
      innerRef: this.refZone
    ...
    }, visibleRegion && visibleZone && React.createElement(SelectedZone, {
      doShowRegions: this.showRegions,
      doShowZones: this.showZones,
      zoneTitle: zoneTitle
    }), visibleRegions && React.createElement(ButtonGroupRegions, {
      doChooseRegion: this.chooseRegion,
      doShowRegion: this.showRegion,
      doShowZones: this.showZones,
      regions: zonesRegions
    }), visibleZones && React.createElement(ButtonGroupZones, {
      allZones: zonesMain,

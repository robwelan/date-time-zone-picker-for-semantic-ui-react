...
import { Button } from 'semantic-ui-react'; //  Utilities

import { getDayOfMonthFromDate, getMonthFromDate, getYearFromDate } from '../../utilities/functions';
...
const DayController = props => {
  ...
  const ButtonGroups = datesArray.map((content, index) => {
    ...
    const classNames = []; // remove month for JavaScript handling of month

    const jsMonth = month - 1;

    if (Number(contentYear) === year && Number(contentMonth) === jsMonth && Number(contentDay) === day) {
      classNames.push('active');
    }

    return (// map content to Button
      React.createElement(Button, {
        className: classNames.join(' ').trim() // nothing unique about processDays
        // eslint-disable-next-line react/no-array-index-key
        ,
        key: index // add month for JavaScript handling of month
        ,
        onClick: setDay(contentYear, contentMonth + 1, contentDay)
      }, React.createElement("span", null, contentDay))
    );
    ...
    return (//  Surround every group with 'Button.Group'

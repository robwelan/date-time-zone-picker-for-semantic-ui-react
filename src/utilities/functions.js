...
const getDateTimeObject = (
  ...
  if (setTime) {
    hour = timeHour;
    minute = timeMinute;
    if (!doTwentyFour && pickerTime.meridiem === 'pm') {
      hour = timeHour + 12;
    }
    if (doSecond) {
      second = timeSecond;
  ...
  return {
    date: {
      ...
      month: pickerDate.month - 1, // JavaScript
      day: pickerDate.day,
    },
    time: {
      hour,
...
const getDateTimeZoneAsOutputObject = (
  setDate,
  setTime,
  setZone,
  pickerDate,
  ...
  const myDateTime = getDateTimeObject(
    setTime,
    pickerDate,
    pickerTime,
    doSecond,
  ...
  const safeDate = getSafeDate(
    myDateTime.date.year,
    myDateTime.date.month,
    myDateTime.date.day,
